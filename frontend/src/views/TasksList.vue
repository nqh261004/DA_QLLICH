    ...
    <div class="space-y-8">
        ...
        <div class="p-4 bg-white rounded-lg shadow-md flex space-x-3 overflow-x-auto">
            <button 
                ...
                class="filter-button"
                :class="{
                    'bg-indigo-600 text-white shadow-lg': currentFilter === status.key,
                    'bg-gray-200 text-gray-700 hover:bg-gray-300': currentFilter !== status.key
                }"
            >
                <component 
                    :is="getStatusDisplay(status.key).icon" 
                    v-if="getStatusDisplay(status.key).icon" 
                    class="w-5 h-5 mr-1" 
                />
                {{ status.label }}
            </button>
        ...
        <div class="bg-white p-6 rounded-lg shadow">
            ...
            <p v-if="isLoading" class="text-center py-8 text-lg">
            ...
            <table v-else class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <tr 
                        ...
                        :key="task.id" 
                        :class="{
                            'bg-red-50 hover:bg-red-100': new Date(task.han_chot) < new Date() && task.trang_thai !== 'phe_duyet',
                    ...
                    >
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span 
                                :class="getStatusDisplay(task.trang_thai).color"
                                class="inline-flex items-center px-2 py-1 text-xs font-semibold rounded-full bg-opacity-20 capitalize"
                            >
                                <component 
                                    :is="getStatusDisplay(task.trang_thai).icon" 
                                    v-if="getStatusDisplay(task.trang_thai).icon" 
                                    class="w-4 h-4 mr-1" 
                                />
